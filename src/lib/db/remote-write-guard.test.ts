...
const LOCAL = 'postgresql://sals3_admin_app:hunter2@localhost:5432/sals3_admin';
const NEON =
  'postgresql://user:s3cr3t@ep-cool-name-123.ap-southeast-2.aws.neon.tech/sals3_admin';
...
describe('classifyDatabaseTarget', () => {
  it.each(['localhost', '127.0.0.1', '0.0.0.0'])(
    ...
    (host) => {
      const target = classifyDatabaseTarget(
        `postgresql://u:p@${host}:5432/sals3_admin`,
      );
  ...
  it('reports an unparseable string distinctly instead of calling it remote', () => {
    // Different problem, different fix - the operator should be told the URL
    // is malformed, not led to believe they are pointed at production.
...
describe('decideRemoteWrite', () => {
  ...
  it('names the blocked host, database, command, and the way out', () => {
    ...
    expect(message).toContain('sals3_admin');
